  ...
  <script>
  	...
  	$(function(){
		...
		function sendMessage(mid){
			let url = "${ctp}/engineer/sendMessage?receiveMid="+mid;
			let widthSize= 450;
			let heightSize = 500;
		...
		function deleteMessage(idx, sw) {
			...
			$.ajax({
				url: "${ctp}/engineer/messageDelete",
				type: "post",
				data: {
					idx : idx,
					sw : sw
		...
		function deleteMessageDB(idx) {
			...
			$.ajax({
				url: "${ctp}/engineer/messageDeleteDB",
				type: "post",
				data: {idx : idx},
				success: function(res) {
					if(res != "0") {
		...
		function midSearchView() {
			...
			let str = "";
			$.ajax({
				url: "${ctp}/engineer/midSearch",
				type: "post",
				data: {mid : mid},
				success: function(eVos) {
					for(let i=0; i<eVos.length; i++) {
						str += '<tr>';
						str += '<td>'+eVos[i].name+'</td>';
						str += '<td><a href="javascript:sendMessage('+eVos[i].mid+')">'+eVos[i].mid+'</td>';
						str += '</tr>';
					}
					$("#memberAll").hide();
			...
			<div class="bodyRight">
				<div class="row mb-2 text-right">
					<div class="col-3 offset-9">
						<a href="#" id="midSearch" data-toggle="modal" data-target="#myModal" class="btn btn-main btn-icon-md btn-round-full ml-3" style="padding:0.45rem;">아이디 검색</a>
					</div>
				</div>
      	<div class="card">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
      	...
      	<table class="table table-hover text-center">
      		...
      		<tbody id="engineerAll">
	      		<c:forEach var="vo" items="${eVos}" varStatus="st">
	      			<tr>
	      				<td>${vo.name}</td>
